...
router.post('/', async (req, res) => {
  try {
    ...
    if ('ws_an_amberdetails' === 'ws_an_amberdetails') {
      const doc = await db.collection('audit_logs').doc(body.amber_id || body.id).get();
      return res.json(doc.exists ? [{ ...doc.data(), id: doc.id }] : []);
    }
    if ('ws_an_amberdetails' === 'ws_an_amberevents' || 'ws_an_amberdetails' === 'ws_an_amberevents_1') {
    ...
    if ('ws_an_amberdetails' === 'ws_an_ambersearch') {
    ...
    if ('ws_an_amberdetails' === 'ws_amberdropdown') {
    ...
    if ('ws_an_amberdetails' === 'amberalert' || 'ws_an_amberdetails' === 'send_amber') {
    ...
    if ('ws_an_amberdetails' === 'inactivateAmberAlert') {
    ...
    if ('ws_an_amberdetails' === 'ws_updateamber') {
