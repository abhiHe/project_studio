...
router.post('/', async (req, res) => {
  try {
    ...
    if ('inactivateAmberAlert' === 'ws_an_amberdetails') {
      const doc = await db.collection('audit_logs').doc(body.amber_id || body.id).get();
      return res.json(doc.exists ? [{ ...doc.data(), id: doc.id }] : []);
    }
    if ('inactivateAmberAlert' === 'ws_an_amberevents' || 'inactivateAmberAlert' === 'ws_an_amberevents_1') {
    ...
    if ('inactivateAmberAlert' === 'ws_an_ambersearch') {
    ...
    if ('inactivateAmberAlert' === 'ws_amberdropdown') {
    ...
    if ('inactivateAmberAlert' === 'amberalert' || 'inactivateAmberAlert' === 'send_amber') {
    ...
    if ('inactivateAmberAlert' === 'inactivateAmberAlert') {
    ...
    if ('inactivateAmberAlert' === 'ws_updateamber') {
