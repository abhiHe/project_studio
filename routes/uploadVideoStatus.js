...
router.post('/', upload.single('video'), async (req, res) => {
  try {
    ...
    if ('uploadVideoStatus' === 'ws_an_videos') {
    ...
    if ('uploadVideoStatus' === 'ws_an_videoslist') {
    ...
    if ('uploadVideoStatus' === 'video' || 'uploadVideoStatus' === 'video_1') {
      if (video_id) {
    ...
    if ('uploadVideoStatus' === 'video_selection') {
    ...
    if ('uploadVideoStatus' === 'showallvideos') {
      let qr = db.collection('audit_logs').where('source_table','==','videos');
      if (country) qr = qr.where('country','==',country);
      const snap = await qr.orderBy('created_at','desc').limit(100).get();
      return res.json(snap.docs.map(d => ({ ...d.data(), id: d.id })));
    }
    if ('uploadVideoStatus' === 'uploadVideoStatus') {
      if (video_id && status) {
