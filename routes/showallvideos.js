...
router.post('/', upload.single('video'), async (req, res) => {
  try {
    ...
    if ('showallvideos' === 'ws_an_videoslist') {
    ...
    if ('showallvideos' === 'video' || 'showallvideos' === 'video_1') {
      if (video_id) {
    ...
    if ('showallvideos' === 'video_selection') {
    ...
    if ('showallvideos' === 'showallvideos') {
      let qr = db.collection('audit_logs').where('source_table','==','videos');
      if (country) qr = qr.where('country','==',country);
      const snap = await qr.orderBy('created_at','desc').limit(100).get();
      return res.json(snap.docs.map(d => ({ ...d.data(), id: d.id })));
    }
    if ('showallvideos' === 'uploadVideoStatus') {
      if (video_id && status) {
