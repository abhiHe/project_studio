...
router.post('/', upload.single('video'), async (req, res) => {
  try {
    ...
    if ('ws_an_videoslist' === 'ws_an_videos') {
    ...
    if ('ws_an_videoslist' === 'ws_an_videoslist') {
    ...
    if ('ws_an_videoslist' === 'video' || 'ws_an_videoslist' === 'video_1') {
      if (video_id) {
    ...
    if ('ws_an_videoslist' === 'video_selection') {
    ...
    if ('ws_an_videoslist' === 'showallvideos') {
      let qr = db.collection('audit_logs').where('source_table','==','videos');
      if (country) qr = qr.where('country','==',country);
      const snap = await qr.orderBy('created_at','desc').limit(100).get();
      return res.json(snap.docs.map(d => ({ ...d.data(), id: d.id })));
    }
    if ('ws_an_videoslist' === 'uploadVideoStatus') {
      if (video_id && status) {
