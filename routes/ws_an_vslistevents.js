...
router.post('/', async (req, res) => {
  try {
    ...
    if ('ws_an_vslistevents' === 'ws_an_approverregister') {
    ...
    if ('ws_an_vslistevents' === 'ws_an_approverlist') {
    ...
    if ('ws_an_vslistevents' === 'ws_an_applyprofile') {
    ...
    if ('ws_an_vslistevents' === 'appliedStatus') {
      const snap = await db.collection('volunteers').where('unique_id','==',uid).limit(1).get();
      if (snap.empty) return res.json(['NoUser']);
    ...
    if ('ws_an_vslistevents' === 'ws_an_vgroupapprove' || 'ws_an_vslistevents' === 'ws_an_vsgroupapprove') {
    ...
    if ('ws_an_vslistevents' === 'ws_an_vsgroupreject') {
    ...
    if ('ws_an_vslistevents' === 'ws_an_vsgroupcase') {
    ...
    if ('ws_an_vslistevents' === 'ws_an_vsgroupaddalert') {
      const docRef = db.collection('audit_logs').doc();
