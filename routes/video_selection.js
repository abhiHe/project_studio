...
router.post('/', upload.single('video'), async (req, res) => {
  try {
    ...
    if ('video_selection' === 'ws_an_videos') {
    ...
    if ('video_selection' === 'ws_an_videoslist') {
    ...
    if ('video_selection' === 'video' || 'video_selection' === 'video_1') {
      if (video_id) {
    ...
    if ('video_selection' === 'video_selection') {
    ...
    if ('video_selection' === 'showallvideos') {
      let qr = db.collection('audit_logs').where('source_table','==','videos');
      if (country) qr = qr.where('country','==',country);
      const snap = await qr.orderBy('created_at','desc').limit(100).get();
      return res.json(snap.docs.map(d => ({ ...d.data(), id: d.id })));
    }
    if ('video_selection' === 'uploadVideoStatus') {
      if (video_id && status) {
