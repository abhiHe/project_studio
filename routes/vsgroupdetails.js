...
router.post('/', async (req, res) => {
  try {
    ...
    if ('vsgroupdetails' === 'ws_an_approverregister') {
    ...
    if ('vsgroupdetails' === 'ws_an_approverlist') {
    ...
    if ('vsgroupdetails' === 'ws_an_applyprofile') {
    ...
    if ('vsgroupdetails' === 'appliedStatus') {
      const snap = await db.collection('volunteers').where('unique_id','==',uid).limit(1).get();
      if (snap.empty) return res.json(['NoUser']);
    ...
    if ('vsgroupdetails' === 'ws_an_vgroupapprove' || 'vsgroupdetails' === 'ws_an_vsgroupapprove') {
    ...
    if ('vsgroupdetails' === 'ws_an_vsgroupreject') {
    ...
    if ('vsgroupdetails' === 'ws_an_vsgroupcase') {
    ...
    if ('vsgroupdetails' === 'ws_an_vsgroupaddalert') {
      const docRef = db.collection('audit_logs').doc();
